...
describe("JS Instrument", () => {
  ...
  it("Captures event listeners", async () => {
    const KEY_LOGGING_URL = `${global.__DEV_SERVER__}/key-logging.html`;
    const page = await browser.newPage();
    const EVENT_RESULTS = [
      [
        "http://localhost:8125/key-logging.html",
        "AddEventListener",
        "keyup",
        6
      ],
    ...
    const eventDataHandler = event => rows.push(event);
    await setupBlacklightInspector(page, eventDataHandler);
    await page.goto(KEY_LOGGING_URL, {
      waitUntil: "networkidle0"
    });
    const testData = [];
    rows.forEach((row: BlacklightEvent) => {
      if (row.type === "AddEventListener") {
        testData.push([
          row["stack"][0]["fileName"],
          row.type,
          row.data.name,
    ...
    expect(testData.sort()).toEqual(EVENT_RESULTS.sort());
    await page.close();
  });
});
