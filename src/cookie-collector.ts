...
export const getJsCookies = (events, url) => {
    return events
        .filter(m => m.type && m.type.includes('JsInstrument.ObjectProperty') && m.data.symbol.includes('cookie') && m.data.operation.startsWith('set') && typeof m.data.value !== 'undefined' && typeof Cookie.parse(m.data.value) !== 'undefined')
        .map(d => {
