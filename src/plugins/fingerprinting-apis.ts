...
export function instrumentFingerprintingApis({ instrumentObjectProperty, instrumentObject, instrumentFunctionViaProxy }) {
    ...
    const navigatorProperties = ['appCodeName', 'appName', 'appVersion', 'clipboard', 'cookieEnabled', 'doNotTrack', 'geolocation', 'language', 'languages', 'onLine', 'platform', 'product', 'productSub', 'userAgent', 'vendorSub', 'vendor'];
    ...
    for (let i = 0; i < window.navigator.mimeTypes.length; i++) {
        const mimeTypeName = (window.navigator.mimeTypes[i] as unknown as MimeType).type; // note: upstream typings seems to be incorrect
    ...
    const excludedProperties = ['quadraticCurveTo', 'lineTo', 'transform', 'globalAlpha', 'moveTo', 'drawImage', 'setTransform', 'clearRect', 'closePath', 'beginPath', 'canvas', 'translate'];
